...
import { CAMPAIGN, VERSUS_LEVEL, levelById } from '../src/content/levels.ts';
...
function newRoom(instance: string | null = null): Room {
  ...
  const room: Room = {
    code, players: [], watchers: [], started: false, level: 'level01', diff: 'standard', mode: 'race',
...
function postReport(
  ...
  // Versus ignores the room's pick — there is one board — so the report has to
  // as well. Reading `room.level` here would file every versus match under
  // whatever sector the lobby happened to be showing, which is Switchback by
  // default and was never played.
  const level = room.mode === 'versus' ? VERSUS_LEVEL : (levelById(room.level) ?? CAMPAIGN[0]!);
